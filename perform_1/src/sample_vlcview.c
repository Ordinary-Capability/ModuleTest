...
#endif
#if defined(CONFIG_CHIP_FH8830) || defined(CONFIG_CHIP_FH8630D) || defined(CONFIG_CHIP_FH8630M)
    {
        .channel = 2,
        .width = CH2_WIDTH,
        .height = CH2_HEIGHT,
...
#if USE_AUDIO_CAP
extern void sample_audio_cap_proc(void *arg);
static rt_thread_t g_thread_audiocap;
#endif

void sample_vlcview_exit(void)
{
	close(sample_vlcview_fd);//record
...
#if USE_AUDIO_CAP
	exit_process = rt_thread_find("vlc_audio_cap");
    if (exit_process)
       rt_thread_suspend(exit_process);
#endif
...
# endif
    ...
    ******************************************/
...
#if USE_AUDIO_CAP
	g_thread_audiocap =
        rt_thread_create("vlc_audio_cap", sample_audio_cap_proc,
                         &g_stop_running, 3 * 1024, RT_APP_THREAD_PRIORITY, 10);
    if (g_thread_audiocap != RT_NULL)
    {
    ...
    g_tcp_conf.cancel = (int *)&g_stop_running;
